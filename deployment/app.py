...
def predict(age, kilometers, fuel_consumption, top_speed, gears, brand, region, 
    ...
    brand_categories =['Abarth', 'Alfa Romeo', 'Aston Martin', 'Audi', 'BMW', 'Chevrolet',
                        ...
                        'Porsche', 'Renault', 'Seat', 'Skoda', 'Subaru', 'Suzuki', 'Toyota', 'Volvo']
    
    if brand == 'VW':
        # If 'VW', all specific dummy variables are 0
        input_data += [0] * len(brand_categories)
    else:
        input_data += [int(brand == b) for b in brand_categories if b != 'VW']
...
with gr.Blocks(theme='snehilsanyal/scikit-learn', title = 'Danish Car Price Predictor', css = css) as app:
    ...
    with gr.Row():
        ...
        with gr.Column():
         ...
         prediction = gr.Text(label="Predicted Price (DKK)")
